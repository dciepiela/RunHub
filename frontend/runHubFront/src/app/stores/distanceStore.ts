...
export default class DistanceStore{
    ...
    setDistance = (distance: DistanceDto) => {
        ...
        if(user){
            distance.isGoing = distance.distanceAttendees!.some(a => a.userName === user.userName)
          }
    ...
    updateDistance = async (raceId: number, distanceId: number, distanceData: Partial<DistanceDto>) => {
        this.loading = true;
        try {
            await agent.Distances.update(raceId, distanceId, distanceData);
            runInAction(() => {
                if(distanceData.distanceId){
                    const updatedDistance = {...this.getDistance(distanceData.distanceId), ...distanceData}
                    this.distanceRegistry.set(distanceId!, updatedDistance as DistanceDto);
    ...
    deleteDistance = async(raceId:number,distanceId:number) => {
        ...
        try {
            ...
            runInAction(() => {
                ...
                // if(this.selectedDistance?.distanceId === distanceId)
                this.loading = false;
            })
  ...
  manualRegister = async (raceId:number, distanceId:number, registrationData:any) => {
    this.loadingInitial = true;
    try {
      await agent.Distances.manualRegistration(raceId, distanceId, registrationData);
      runInAction(() => {
        this.loadingInitial = false;
      });
    } catch (error) {
  ...
  registerAttendeeWithPayment = async (raceId: number, distanceId: number, stripeToken: string, amount: number) => {
    ...
    try {
        ...
        router.navigate('/payment-success', { state: { message: 'Płatność pomyślna, zostałeś zapisany na bieg!', raceId:raceId } });
    } catch (error) {
  ...
  toggleIsReadyToShow = async (distanceId: number, isReadyToShow: boolean) => {
    ...
    try {
        ...
        runInAction(() => {
            ...
            if (distance) {
                console.log("Updating distance in registry", distanceId, isReadyToShow); // Debugging log
  ...
  generateDistanceIncomeReport = async (distanceId: number) => {
    ...
    try {
        const reportData = await agent.Distances.generateDistanceIncomeReport(distanceId);
        runInAction(() => {
        ...
        console.error("Error generating report for distance:", error);
    }
};
